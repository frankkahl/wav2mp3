...
regex Guid::guid_regexp =
    std::regex(R"([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})", std::regex::icase);
...
string Guid::string() const {
    ...
    ss << right << setfill('0') << setw(8) << hex << time_low << '-' << setw(4) << time_mid << '-' << setw(4)
       << time_hi_and_version << '-' << setw(2) << (uint16_t)clock_seq_low << setw(2) << (uint16_t)clock_seq_hi << '-';
    for (int i = 0; i < 6; i++) {
...
bool operator==(const Guid &a, const Guid &b) {
    return (a.time_low == b.time_low) && (a.time_mid == b.time_mid) && (a.time_hi_and_version == b.time_hi_and_version)
           && (a.clock_seq_low == b.clock_seq_low) && (a.clock_seq_hi == b.clock_seq_hi)
