...
static fs::recursive_directory_iterator is_directory_valid(const string &passed_path) {
	...
	try {
		...
		// Being relative and begin with a path separator can happen only under Windows when the dreaded drive letter is omitted
		// In this case convert it to a normalized absolute path
		if (path.is_relative()
			&& (path.lexically_normal().wstring()[0] != fs::path::preferred_separator)) {
			path = fs::proximate(path);  // relative() would convert a path without any separators to an empty string, so proximate() is used
		}
		else {
